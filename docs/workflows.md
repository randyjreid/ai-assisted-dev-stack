...
Prompts must be clear. Each request specifies a single outcome, the files in scope, the acceptance criteria, and any relevant constraints or safety instructions. Ambiguous requests such as “clean up this module” are replaced with concrete statements of what to change, what to avoid, and how success will be verified. Reproducibility matters just as much: prompts include branch names, commit hashes when necessary, and references to previous outputs so the same scenario can be re-run later without guesswork.

Transparency and auditability require prompts to cite their source. If a bug report triggered the work, the prompt links to that issue and explains the impact. When documentation informs the change, the prompt states which sections should be considered. Every significant prompt is stored in an issue comment, pull request discussion, or the prompt log so reviewers can see how the AI was directed. Safety guardrails are embedded throughout. Prompts explicitly list the files the AI may touch, discourage speculation, and remind the model to surface uncertainties rather than inventing answers. They also describe how the output will be validated, such as running a particular test suite or comparing behavior against the current design.

Verifiable output is the final principle. Prompts instruct the model to produce diffs, test updates, or step-by-step reasoning so humans can confirm the work. When a prompt requires analysis rather than code, it asks for structured findings and follow-up actions instead of generic commentary. Throughout this process developers compare potential prompts before sending them, choosing the version that is easiest to verify later and noting poor examples in the playbook so the team can avoid them.
...
Context persists across sessions by saving reusable blocks in the playbook, reusing markdown snippets that describe subsystems, and tagging prompt logs with the files involved. When prompt history becomes tangled or conflicting instructions appear, the developer resets the session and rebuilds the context from the latest source of truth. That prevents stale assumptions or partial histories from steering later prompts. Resetting is also required after large refactors, when switching between unrelated tasks, or when the AI output shows signs of contamination (for example, referencing files that were never mentioned). Treating context as a first-class artifact makes it easier to share reasoning with reviewers and keeps each AI interaction grounded in the latest information.

### 2.4 Claude Code Prompt Patterns
Claude Code handles structured, repository aware tasks, so prompts follow a consistent template. Every request opens with a summary, the current branch and commit, and a list of files that may be edited. A context block includes code snippets, diffs, or design notes. Requirements and acceptance criteria describe what must change and which tests or commands prove success. Constraints and safety directions close the prompt by declaring what the model must not do.

Multi file edits and repository wide scans rely on that structure to limit scope while still giving Claude access to relevant information. Implementation tasks spell out function level behavior, input and output expectations, and testing hooks. Documentation prompts describe tone, link targets, and any cross references to update. Refactoring prompts emphasize invariants to preserve and often require the model to describe the planned sequence before applying changes. Each prompt ends with explicit instructions about patch formatting and human review so changes enter the Git workflow cleanly.
...
Prompts move through the same lifecycle as code changes. During capture, the developer records the intent, source issue, and initial context. The refine stage tightens wording, adds constraints, and often includes a quick peer review for high risk work. Execute covers the actual interaction with Claude or Codex and includes saving the prompt and output together. Evaluate means running tests, reviewing diffs, and deciding whether a follow-up prompt is required. Archive closes the loop by storing the prompt, the resulting change, and any lessons learned in the issue tracker, pull request discussion, or prompt log.

Maintaining this lifecycle keeps prompts traceable. Reviewers can trace a commit back to the prompt that produced it, see the context that was supplied, and understand why a particular approach was chosen. When prompts fail, the archive shows what went wrong and informs better instructions next time.
...
This section describes the philosophy and flow; the actionable templates live in the Prompting Playbook. The playbook contains reusable context blocks, safety checklists, and model specific templates referenced throughout Section 2. As new prompt patterns emerge from active projects, they are added to the playbook and linked from the relevant lifecycle stage (planning, implementation, testing, or review). The combination of this narrative and the playbook keeps prompting both consistent and easy to evolve.
